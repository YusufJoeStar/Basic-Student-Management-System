...
class School{
    ...
    int ID;
    School () {}
    School(int id, double g, string n) : ID(id), grade(g), name(n) {}
...
int ID_Finder(School arr[], int n, int id) {
    for (int i = 0; i < n; i++) {
        if (arr[i].ID == id)
            return i;
    }
...
int Student_Finder_Name(School arr[], int n, string x) {
    for(int i = 0; i < n; i++) {
        if(arr[i].name == x)
            return i;
    }
...
//Deleting a student
int delete_Student(School arr[], int &n, int idToDelete) {
    int pos = ID_Finder(arr, n, idToDelete);
    if (pos == -1)
        return -1;
...
//Editing the data of a student
int Edit(School arr[], int n, int idToEdit) {
    int pos = ID_Finder(arr, n, idToEdit);
    if (pos == -1)
        return -1;
...
//Calculating the average, top student, lowest student
...
int main(){
   ...
   int nextId = 1000; // starting ID, ID's automatically generated 1000,1001,etc..
...
for (int i = 0; i < n; i++) {
    ...
    student[i] = School(nextId++, grade, name);  // increment ID
}
    ...
    do {
        ...
        switch (choice) {
            ...
            case 3: {
                ...
                int index = Student_Finder_Name(student, n, searchName);
                if (index != -1) {
                    cout << "Student found: ";
            ...
            case 5: {
                ...
                int result = delete_Student(student, n, id);
                if (result == -1)
